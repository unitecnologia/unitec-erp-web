...
class ErpUpdateService
{
    ...
    {
        $updater = rtrim((string) base_path(), '\\/').DIRECTORY_SEPARATOR.'bin'.DIRECTORY_SEPARATOR.'Unitec Atualizador.exe';
        $viaUpdater = PHP_OS_FAMILY === 'Windows' && is_file($updater) && self::isLocalPackageReady();
        ...
        self::writeStatus(
            ...
            $viaUpdater
                ? 'bin\\Unitec Atualizador.exe --zip (pacote local)'
    ...
    public function downloadPendingUpdate(string $appPath, bool $force = false): array
    {
        ...
        try {
            ...
            if (! $force && self::isLocalPackageReady()) {
                ...
                if ($keepReady) {
                    self::writePackageMeta([
                        ...
                        'check_message' => 'Pacote '.$keepLabel.' já baixado. Execute a atualização manual.',
                    ...
                    return [
                        'downloaded' => false,
                        'message' => 'Pacote já está pronto. Execute a atualização manual ('.$keepLabel.').',
                        'remote_version' => $remoteVersion ?? $readyVersion,
                    ];
                }
            }
            ...
            self::writePackageMeta([
                ...
                'remote_version' => $remoteVersion,
                'local_version' => $localVersion,
                'check_message' => 'Pacote pronto. Execute a atualização manual'.($remoteVersion ? ' ('.$remoteVersion.')' : '').'.',
                'last_check_at' => now()->toIso8601String(),
            ]);
    ...
    public function run(string $appPath): void
    {
        ...
        try {
            ...
            $packageVersion = $this->readVersionFromDisk($sourceRoot);
            if ($packageVersion !== '' && $remoteFromManifest !== '' && $packageVersion !== $remoteFromManifest) {
                $this->log(
                    $appPath,
                    "Aviso: manifest={$remoteFromManifest} mas config do pacote={$packageVersion}; usando config do pacote."
                );
            }
            ...
            // Config do pacote manda; manifest só se config estiver vazio.
            $expectedVersion = $packageVersion !== '' ? $packageVersion : $remoteFromManifest;
    ...
    private function copyDirectory(
        ...
        foreach ($iterator as $item) {
            ...
            $copiedOk = $this->copyFileReplacing($sourcePath, $destination, $relative, $target);
            if ($onFileCopied !== null) {
                $onFileCopied(! $copiedOk);
            }
        }
    }

    /**
     * Copia arquivo; se o destino estiver em uso (Atualizador/serviço), mantém o atual e segue.
     *
     * @return bool true se gravou no destino final; false se pulou (arquivo em uso)
     */
    private function copyFileReplacing(string $sourcePath, string $destination, string $relative, string $copyTargetRoot): bool
    {
        try {
            ...
            if (! $this->isCopyBusyError($e)) {
                throw $e;
            }

            $this->log(
                $this->guessAppPathFromCopyTarget($copyTargetRoot),
                "Arquivo em uso (mantido): {$relative} — ".$e->getMessage()
            );
    ...
    private function isCopyBusyError(\Throwable $e): bool
    {
        $message = strtolower($e->getMessage());

        return str_contains($message, 'resource temporarily unavailable')
            || str_contains($message, 'being used by another process')
            || str_contains($message, 'text file busy')
            || str_contains($message, 'usado por outro processo');
    }

    private function guessAppPathFromCopyTarget(string $copyTargetRoot): string
    {
        $normalized = rtrim(str_replace(['/', '\\'], DIRECTORY_SEPARATOR, $copyTargetRoot), DIRECTORY_SEPARATOR);
        if (str_ends_with(strtolower($normalized), DIRECTORY_SEPARATOR.'vendor')
            || str_ends_with(strtolower($normalized), '\vendor')) {
