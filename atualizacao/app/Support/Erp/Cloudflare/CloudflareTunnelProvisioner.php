...
final class CloudflareTunnelProvisioner
{
    ...
    public function provision(Empresa $empresa, array $overrides = []): array
    {
        ...
        if ($existingTunnelId !== '') {
            ...
            $created = $this->createLocalTunnel($creds, $this->tunnelName($empresa, $subdomain));
            $tunnelId = $created['id'];
    ...
    private function tunnelName(Empresa $empresa, string $subdomain): string
    {
    ...
    private function writeLocalFiles(
        ...
        if ($tunnelSecret !== null) {
            ...
            throw new RuntimeException(
                "Túnel {$tunnelId} já cadastrado, mas falta o arquivo de credencial em {$credentialsPath}. Remova o Tunnel ID na empresa e provisione de novo, ou restaure o JSON."
            );
        }
