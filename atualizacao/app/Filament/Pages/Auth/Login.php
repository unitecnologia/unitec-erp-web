...
class Login extends BaseLogin
{
    ...
    public function authenticate(): ?LoginResponse
    {
        ...
        // Atualização pronta em atualizacao/: pergunta antes da licença.
        if (AtualizacaoPasta::isPendingNewer()) {
    ...
    public function aceitarAtualizacao(): void
    {
        ...
        $this->js('window.UnitecLoginBoot && window.UnitecLoginBoot.show && window.UnitecLoginBoot.show('.Js::from('Aplicando atualização…').')');

        try {
            app(AtualizacaoApplyService::class)->apply(base_path());
        } catch (\Throwable $e) {
            Log::error('Falha ao aplicar atualizacao/', ['message' => $e->getMessage()]);
            $this->applyingUpdate = false;
            $this->js('window.UnitecLoginBoot && window.UnitecLoginBoot.hide && window.UnitecLoginBoot.hide()');
            throw ValidationException::withMessages([
                'data.login_senha' => 'Não foi possível aplicar a atualização: '.$e->getMessage(),
            ]);
        }
    ...
    private function finishLoginAfterUpdateChoice(): void
    {
