...
trait ErpOrcamentoFormPage
{
    ...
    protected function loadOrcamentoFormFromRecord(Orcamento $orcamento): void
    {
        ...
        $isCf = $cliente && Person::isCodigoConsumidorFinal($cliente->codigo);
        ...
        if ($isCf && $hasSnapshot) {
            ...
            $this->applyClienteFields($cliente, enterAvulso: $isCf && ! $hasSnapshot);
        }
     ...
     * - nome cadastrado (exato) → carrega cadastro e vai ao produto;
     * - nome NÃO cadastrado → libera campos manuais e pula para o CPF;
    ...
    public function handleClienteEnter(): void
    {
        ...
        // Usuário navegou nas sugestões com seta e o item destacado NÃO é o texto digitado → confirma a sugestão.
        if (
            $this->clienteLookupOpen
            && $this->selectedClienteIndex !== null
            && isset($this->clienteResults[$this->selectedClienteIndex])
        ) {
            $row = $this->clienteResults[$this->selectedClienteIndex];
            $rowNome = mb_strtoupper(trim((string) $row['nome']), 'UTF-8');
            $rowFantasia = mb_strtoupper(trim((string) ($row['fantasia'] ?? '')), 'UTF-8');
            $rowDoc = preg_replace('/\D/', '', (string) ($row['cpf_cnpj'] ?? '')) ?? '';
            $nomeDigits = preg_replace('/\D/', '', $nome) ?? '';
            $highlightedIsTyped = $rowNome === $nome
                || ($rowFantasia !== '' && $rowFantasia === $nome)
                || ($nomeDigits !== '' && $rowDoc !== '' && $rowDoc === $nomeDigits);

            if (! $highlightedIsTyped && $this->selectedClienteIndex > 0) {
                $this->confirmClienteSelection();
        ...
        // Nome sem cadastro exato → modo avulso (dados manuais no orçamento).
    ...
    {
        // Não confirma sugestão no blur — Enter / clique escolhem o cliente.
        // Assim um nome novo não vira o primeiro resultado parecido.
    ...
    public function confirmarCodigoProdutoBar(): void
    {
        if ($this->orcamentoReadOnly()) {
            return;
        }
        ...
        if ($this->produtoLookupOpen && $this->produtoResults !== []) {
            $index = $this->selectedProdutoIndex ?? 0;
            $this->selectProdutoResult($index);
        ...
        $product = $this->findProductByCodigo(mb_strtoupper($term, 'UTF-8'))
            ?? $this->findProductByTerm(mb_strtoupper($term, 'UTF-8'));
