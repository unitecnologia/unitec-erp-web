...
trait ManagesEmpresaFormUi
{
    ...
    /** @var array{online: bool, checked_at: ?string, last_online_at: ?string, pid: ?int, message: string, source: ?string} */
    ...
    public function startWhatsAppNodeService(): void
    {
        ...
        try {
            $manager->restartGateway($empresa->fresh());
        } catch (\Throwable $e) {
            Notification::make()
                ->title('WhatsApp')
        ...
        $ok = $manager->isHealthy(WhatsAppConfig::fromEmpresa($empresa->fresh()));

        Notification::make()
            ->title('WhatsApp')
            ->body($ok
                ? 'Serviço Node iniciado e respondendo.'
                : 'Serviço iniciado, mas ainda não respondeu. Aguarde alguns segundos e use Atualizar status.')
            ->{$ok ? 'success' : 'warning'}()
            ->send();
    }
