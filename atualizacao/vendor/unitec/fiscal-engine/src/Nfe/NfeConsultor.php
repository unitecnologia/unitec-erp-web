...
final class NfeConsultor
{
    ...
    public function consultar(ConsultarNfceRequest $request): ConsultarNfceResponse
    {
        ...
        if (strlen($chave) !== 44) {
            throw new FiscalEngineException('Chave de acesso inválida para consulta da NF-e.');
        }
        ...
        $endpoint = ScNfeEndpoints::consultaProtocolo($request->tpAmb);
    ...
    private function buildConsSitNfe(int $tpAmb, string $chave): string
    {
        ...
        if (! $root instanceof \DOMElement) {
            throw new FiscalEngineException('Não foi possível montar a consulta da NF-e.');
        }
