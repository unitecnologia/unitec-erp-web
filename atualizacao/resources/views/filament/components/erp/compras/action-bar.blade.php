<div class="erp-compras-actions">
    <button
        ...
        data-erp-key="F2"
        title="Dar entrada de mercadoria pelo XML da NF-e"
    >
    ...
    <button
        ...
        data-erp-key="F4"
        @disabled($this->highlightedCompraDevolvida)
        title="{{ $this->highlightedCompraDevolvida ? 'Compra devolvida não pode ser cancelada' : 'Cancelar compra' }}"
    >
    ...
    <button
        type="button"
        wire:click="openLerXmlFromCompraSelecionada"
        class="erp-compras-actions__btn"
        data-erp-key="F6"
        @disabled($this->highlightedCompraDevolvida)
        title="{{ $this->highlightedCompraDevolvida ? 'Compra devolvida não permite leitura de XML' : 'Importar XML (arquivo do e-mail/pasta). Compra já lançada: use Cancelar (F4) para estornar.' }}"
    >
        <span class="erp-compras-actions__icon">📄</span>
