@php
    ...
    $columnTotals = AjusteEstoqueListagemReport::columnTotals($ajustes, $columns);
    $hasRows = count($ajustes) > 0;
@endphp
<div class="pessoa-list-doc">
    <div class="pessoa-list-doc__frame">
        ...
        <div class="pessoa-list-doc__filters">
            <span>| PERÍODO: {{ mb_strtoupper($periodLabel, 'UTF-8') }}</span>
            @if ($searchLabel)
        ...
        <table class="pessoa-list-doc__table">
            <thead>
                <tr>
                    @foreach ($columns as $column)
                        <th class="{{ AjusteEstoqueListagemReport::isNumericColumn($column) ? 'num' : '' }}">
            ...
            <tbody>
                @forelse ($ajustes as $ajuste)
                    <tr>
                        @foreach ($columns as $column)
                            <td class="{{ AjusteEstoqueListagemReport::isNumericColumn($column) ? 'num' : '' }} {{ $column === 'produto' ? 'nome' : '' }}">
                                {{ AjusteEstoqueListagemReport::cellValue($ajuste, $column) }}
                            </td>
                        @endforeach
                    </tr>
                @empty
                    <tr>
                        <td colspan="{{ count($columns) }}" class="pessoa-list-doc__empty">Nenhum ajuste encontrado.</td>
                    </tr>
                @endforelse
            </tbody>
            @if ($hasRows)
                <tfoot>
                    <tr class="pessoa-list-doc__totals">
                        @foreach ($columns as $column)
                            <td class="{{ AjusteEstoqueListagemReport::isNumericColumn($column) ? 'num' : '' }}">
                                {{ $columnTotals[$column] }}
                            </td>
                        @endforeach
                    </tr>
