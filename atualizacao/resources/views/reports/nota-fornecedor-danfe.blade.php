...
<head>
    ...
    <style>
        ...
        * {
        ...
        .viewer--embed .viewer__canvas {
            padding: 0.5rem;
            overflow: auto;
        }

        .viewer--embed .viewer__paper {
            width: min(210mm, 100%);
            margin: 0 auto;
            box-shadow: none;
            padding: 3mm;
...
<body @class([
    ...
    'viewer-body--embed' => ($embedded ?? false) && ! $autoPrint,
])>
    <div @class([
        'viewer',
        'viewer--auto-print' => $autoPrint,
        'viewer--embed' => ($embedded ?? false) && ! $autoPrint,
    ])>
        @unless ($autoPrint || ($embedded ?? false))
    ...
    <script>
        const pdfDownloadUrl = @json(route('erp.reports.nota-fornecedor-danfe', ['nota' => $nota->id, 'pdf' => 1]));
        ...
        if (autoPrint) {
            window.addEventListener('afterprint', () => {
                if (window.parent !== window) {
                    window.parent.postMessage({ type: 'erp-nf-forn-danfe-print-done' }, '*');
