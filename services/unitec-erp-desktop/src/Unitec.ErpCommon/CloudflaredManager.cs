...
public static class CloudflaredManager
{
    ...
    public static void Ensure(string appPath)
    {
        lock (Gate)
        {
            try
            {
                ...
                var restartRequested = File.Exists(ErpPaths.CloudflaredRestartFlagPath);
                if (restartRequested)
                {
                    DesktopLog.Write(appPath, "cloudflared: restart.flag — reiniciando túnel");
                    TryStopManagedProcess();
                    TryDeletePidFile();
                ...
                if (!File.Exists(ErpPaths.CloudflaredConfigPath))
                {
                    WriteStatus(
                        online: false,
                        pid: null,
                        message: "Túnel ainda não criado — clique em Ativar túnel na tela da empresa.");
                    return;
                }
                ...
                if (!File.Exists(exe) && exe == "cloudflared")
                {
                    WriteStatus(
                        online: false,
                        pid: null,
                        message: "cloudflared.exe nao encontrado (resources\\cloudflared no pacote).");
                    return;
                }
    ...
    public static void RefreshStatusOnly()
    {
        lock (Gate)
        {
            ...
            WriteStatus(
                online: false,
                pid: null,
                message: "Config OK, mas cloudflared parado — reinicie o UnitecErpServer");
        }
    }

    private static void TryStopManagedProcess()
    {
        if (!TryGetManagedPid(out var pid) || !IsProcessAlive(pid))
        {
            return;
    ...
    private static void TryDeleteRestartFlag()
    {
        try
        {
            if (File.Exists(ErpPaths.CloudflaredRestartFlagPath))
            {
                File.Delete(ErpPaths.CloudflaredRestartFlagPath);
            }
        }
        catch
        {
