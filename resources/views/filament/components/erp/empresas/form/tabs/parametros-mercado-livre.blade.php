@php
    ...
    $hubUrl = app(\App\Support\MercadoLivre\MeliHubService::class)->hubBaseUrl();
...
>
    ...
    <details class="erp-portal-contador-advanced" open>
        ...
        <p class="erp-empresas-parametros__hint">
            Estes valores gravam no <code>.env</code>. No servidor Unitec deixe
            <strong>Este servidor é o hub</strong> marcado e o APP_URL com HTTPS público.
        </p>

        <div class="erp-empresas-parametros__checks erp-empresas-parametros__checks--inline">
            <label class="erp-pcad__check">
                <input type="checkbox" wire:model="data.meli_env_is_hub">
                <span>Este servidor é o hub (MELI_IS_HUB)</span>
            </label>
        </div>
        ...
        <div class="erp-empresas-parametros__form-grid erp-empresas-parametros__form-grid--wide">
            <div class="erp-empresas-parametros__field">
                <label class="erp-pcad-form__label" for="meli-env-app-url">APP_URL</label>
                <input
                    id="meli-env-app-url"
                    type="text"
                    wire:model="data.meli_env_app_url"
            ...
            <div class="erp-empresas-parametros__field">
                <label class="erp-pcad-form__label" for="meli-env-hub-url">Hub URL</label>
                <input
                    id="meli-env-hub-url"
                    type="text"
                    wire:model="data.meli_env_hub_url"
                    class="erp-pcad-form__input erp-pcad-form__input--grow"
                    placeholder="https://unitecnologiasc.com.br"
                    autocomplete="off"
                    spellcheck="false"
        ...
        <div class="erp-empresas-parametros__actions" style="margin-top: 0.75rem;">
            <button
                type="button"
                class="erp-pcad-form__btn erp-portal-contador-vinculo-panel__btn-primary"
                wire:click="salvarMercadoLivreAvancado"
                wire:loading.attr="disabled"
                wire:target="salvarMercadoLivreAvancado"
            >
                <span wire:loading.remove wire:target="salvarMercadoLivreAvancado">Salvar no .env</span>
                <span wire:loading wire:target="salvarMercadoLivreAvancado">Salvando…</span>
            </button>
        </div>
    </details>
</div>
