...
@php
    $nfeSucessoVisivel = filled($this->nfeFiscalSucessoDetalhe)
        && ! $this->nfeWhatsAppModalOpen
        && ! $this->nfeDanfeEmailModalOpen
        && ! $this->nfeCancelModalOpen
        && ! $this->nfeInutilizarModalOpen
        && ! $this->nfeCceModalOpen
        && ! filled($this->nfeCceSucessoDetalhe);
@endphp
<div
    id="erp-nfe-fiscal-sucesso-overlay"
    class="erp-nfe-fiscal-overlay erp-nfe-fiscal-overlay--sucesso{{ $nfeSucessoVisivel ? ' is-visible' : '' }}"
    role="alertdialog"
    aria-labelledby="erp-nfe-fiscal-sucesso-title"
    aria-live="polite"
    aria-hidden="{{ $nfeSucessoVisivel ? 'false' : 'true' }}"
    style="display: {{ $nfeSucessoVisivel ? 'grid' : 'none' }};"
    data-erp-nfe-fiscal-sucesso-overlay
>
