...
<div class="fi-ta-ctn overflow-x-auto" wire:key="erp-produtos-table-{{ $isSeriais ? 'seriais' : 'produtos' }}-{{ $this->statusFilter }}-{{ $this->searchColumn }}-{{ md5($this->localSearch) }}-p{{ $this->getTablePage() }}">
                    ...
                    >
                        @if ($column['sortable'] ?? false)
                            <button
                                ...
                                wire:click="sortTable('{{ $column['key'] }}')"
                            >{{ $column['label'] }}</button>
                        @else
                            {{ $column['label'] }}
                        @endif
        ...
        <tbody>
            @forelse ($records as $index => $record)
                @php
                    ...
                    $selected = (int) $this->highlightedRecordId === (int) $recordKey;
                    $rowClass = trim('fi-ta-row ' . ($index % 2 === 1 ? 'fi-striped ' : '') . ($selected ? 'erp-row-selected' : ''));
                @endphp
                <tr
                    class="{{ $rowClass }}"
                    data-record-key="{{ $recordKey }}"
                    wire:key="erp-prod-row-{{ $recordKey }}"
                >
                    @if ($isSeriais)
                        <td class="fi-ta-cell"><div class="fi-ta-col"><span class="fi-ta-text">{{ $record->product?->descricao }}</span></div></td>
                        <td class="fi-ta-cell text-center"><div class="fi-ta-col"><span class="fi-ta-text">{{ $record->numero_serie }}</span></div></td>
                        <td class="fi-ta-cell text-center"><div class="fi-ta-col"><span class="fi-ta-text">{{ $record->situacao }}</span></div></td>
                        <td class="fi-ta-cell text-center"><div class="fi-ta-col"><span class="fi-ta-text">{{ $record->doc_saida ?: '—' }}</span></div></td>
                        <td class="fi-ta-cell text-center"><div class="fi-ta-col"><span class="fi-ta-text">{{ $record->data_baixa ? \Illuminate\Support\Carbon::parse($record->data_baixa)->format('d/m/Y') : '—' }}</span></div></td>
                    @else
                        @php
                            /** @var Product $record */
                            ...
                            <td @class(['fi-ta-cell', 'text-' . $column['align'] => filled($column['align'] ?? null)])>
                                <div class="fi-ta-col">
                                    @if ($column['html'] ?? false)
                                        <span class="fi-ta-text">{!! $value !!}</span>
                                    @else
                                        <span class="fi-ta-text">{{ $value }}</span>
                                    @endif
                                </div>
            ...
            @empty
                <tr class="fi-ta-row">
                    <td class="fi-ta-cell" colspan="{{ count($columns) }}">
                        <div class="fi-ta-col">
                            <span class="fi-ta-text">{{ $isSeriais ? 'Nenhum serial encontrado' : 'Nenhum produto encontrado' }}</span>
                        </div>
                    </td>
                </tr>
            @endforelse
    ...
    @if ($records instanceof \Illuminate\Contracts\Pagination\Paginator && $records->hasPages())
        <nav class="fi-pagination mt-2 px-2" aria-label="Paginação">
            <div class="flex items-center gap-2 text-sm">
                ...
                @else
                    <button type="button" class="erp-produtos__btn erp-produtos__btn--secondary" wire:click="previousPage('{{ $pageName }}')">Anterior</button>
                @endif

                <span>Página {{ $records->currentPage() }} de {{ $records->lastPage() }}</span>

                @if ($records->hasMorePages())
                    <button type="button" class="erp-produtos__btn erp-produtos__btn--secondary" wire:click="nextPage('{{ $pageName }}')">Próxima</button>
                @else
