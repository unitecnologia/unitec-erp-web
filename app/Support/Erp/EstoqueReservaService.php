...
final class EstoqueReservaService
{
   ...
   */
  public function totaisReservadosAtivos(): array
  {
  ...
  public function reservadoAtivo(int $productId): float
  {
    return (float) EstoqueReserva::query()
      ->where('product_id', $productId)
      ->where('status', EstoqueReserva::STATUS_ATIVA)
      ->sum('quantidade');
  }

  public function disponivel(Product $product, ?int $ignorarOrderId = null): float
  {
    $fisico = (float) $product->estoque;
    $reservado = $this->reservadoAtivoParaProduto($product->id, $ignorarOrderId);

    return $fisico - $reservado;
  }

  public function reservadoAtivoParaProduto(int $productId, ?int $ignorarOrderId = null): float
  {
   ...
   */
  public function reservasAtivasDoProduto(int $productId): Collection
  {
    return EstoqueReserva::query()
      ->where('product_id', $productId)
      ->where('status', EstoqueReserva::STATUS_ATIVA)
  ...
  private function validarDisponivel(
    ...
    if ($product->is_composicao) {
      $erro = $this->stock->validaEstoqueComposicao($product, $quantidade);

      return $erro !== null ? $erro : null;
    }

    $disponivel = $this->disponivel($product);
