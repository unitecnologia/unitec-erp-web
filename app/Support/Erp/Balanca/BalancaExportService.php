...
final class BalancaExportService
{
     ...
     * PLU no arquivo = codigo_barras curto (≤6); se EAN (≥8), PLU = codigo e EAN no campo próprio.
    ...
    protected function scaleCode(Product $product, int $digitos = 6): string
    {
        ...
        // com zeros à esquerda (ex.: 207 → 000207).
        $fileWidth = BalancaProductRules::PLU_DIGITOS;

        $barras = preg_replace('/\D/', '', (string) ($product->codigo_barras ?? '')) ?? '';

        // PLU curto em codigo_barras; EAN (≥8) fica no campo EAN — PLU usa codigo.
        if ($barras !== '' && strlen($barras) <= $fileWidth) {
            $raw = $barras;
        } else {
