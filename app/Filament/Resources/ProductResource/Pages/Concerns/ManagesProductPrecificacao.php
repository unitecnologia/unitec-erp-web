...
trait ManagesProductPrecificacao
{
    ...
    public function openProductPrecificacaoFromData(array $data): void
    {
        ...
        $custoSalvo = BrDecimal::parse($data['preco_custo'] ?? 0, 2);
        if ($compra <= 0 && $custoSalvo > 0) {
            $compra = $custoSalvo;
        }
    ...
    public function switchPrecificacaoEmpresa(int|string|null $empresaId): void
    {
        ...
        if (! $this->productPrecificacaoOpen || $this->precificacao === [] || $empresaId <= 0) {
            return;
        }

        if ($empresaId === $this->currentProductEmpresaId()) {
            return;
        }

        $this->applyPrecificacaoToHost();
        $this->switchProductFormEmpresa($empresaId);
        $this->openProductPrecificacaoFromData(is_array($this->data) ? $this->data : []);
        $this->touchPrecificacao();
