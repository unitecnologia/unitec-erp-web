...
class ListCompras extends ListRecords
{
    ...
    /** @var array<int, array{id: int, codigo: string, nome: string, cnpj: string}> */
    public array $lancamentoRomaneioFornecedores = [];

    public int $lancamentoRomaneioFornecedorSelecionado = 0;

    public bool $lancamentoRomaneioFornecedorConfirmado = false;

    public bool $lancamentoRomaneioFornecedorBuscaSilent = false;
    ...
    public string $lancamentoLotesProdutoLabel = '';

    public string $lancamentoLotesQtdEsperada = '0,000';

    /** @var list<array{lote: string, data_validade: string, quantidade: string}> */
    public array $lancamentoLotesDraft = [];
    ...
    /**
     * Empresas para o seletor do modal de precificação (compartilhado com o cadastro de produto).
     *
    ...
    /**
     * No lançamento de compra não há formulário de produto por empresa — troca é no-op.
     */
    ...
    /**
     * Lançamento não troca empresa no modal de precificação.
     * Evita reabrir com $data vazio (stub) e zerar preços/margens.
     */
    public function switchPrecificacaoEmpresa(int|string|null $empresaId): void
    {
        //
    }
    ...
    public function openCompraLancamento(int $compraId, string $modo = 'visualizar'): void
    {
        ...
        if ($modo === 'alterando' && $compra->status === Compra::STATUS_FECHADA) {
            Notification::make()
                ->title('Compra fechada')
                ->body('Para alterar, use F7 Reabrir (estorna estoque/financeiro). Abrindo em visualização.')
                ->warning()
                ->send();
        ...
        if ($this->lancamentoRomaneio) {
            $nomeFornecedor = trim((string) ($compra->fornecedor?->nome_razao ?? ''));
            $this->lancamentoRomaneioFornecedorBuscaSilent = true;
            $this->lancamentoRomaneioFornecedorBusca = $nomeFornecedor;
            $this->lancamentoRomaneioFornecedorBuscaSilent = false;
            $this->lancamentoRomaneioFornecedores = [];
            $this->lancamentoRomaneioFornecedorSelecionado = 0;
            $this->lancamentoRomaneioFornecedorConfirmado = $nomeFornecedor !== '';
    ...
    public function iniciarEntradaRomaneio(int $fornecedorId): void
    {
        ...
        $this->lancamentoRomaneioFornecedores = [];
        $this->lancamentoRomaneioFornecedorSelecionado = 0;
        $this->lancamentoRomaneioFornecedorConfirmado = false;
        ...
        $this->lancamentoModalHeader['fornecedor'] = '—';
        $this->lancamentoModalHeader['uf'] = '—';
        $this->lancamentoModalHeader['cnpj'] = '—';
        $this->dispatch('erp-compra-romaneio-focus-fornecedor');
    }

    public function updatedLancamentoRomaneioFornecedorBusca(): void
    {
        if ($this->lancamentoRomaneioFornecedorBuscaSilent) {
            return;
        }
        ...
        $this->lancamentoRomaneioFornecedores = Person::query()
            ->where('ativo', true)
            ->where('is_fornecedor', true)
            ->where(function (Builder $query) use ($like, $digits, $term): void {
            ...
            ->map(fn (Person $pessoa): array => [
                'id' => (int) $pessoa->id,
                'codigo' => (string) ($pessoa->codigo ?? ''),
                'nome' => (string) $pessoa->nome_razao,
                'cnpj' => $this->formatCpfCnpj($pessoa->cpf_cnpj),
            ])
            ->all();
    ...
    public function moveLancamentoRomaneioFornecedorSelecionado(int $direction): void
    {
        $count = count($this->lancamentoRomaneioFornecedores);
        if ($count === 0) {
            return;
        }

        $this->lancamentoRomaneioFornecedorSelecionado = max(
            0,
            min($count - 1, $this->lancamentoRomaneioFornecedorSelecionado + $direction),
        );
    }

    public function confirmarLancamentoRomaneioFornecedor(?string $value = null): void
    {
        if ($this->lancamentoRomaneioFornecedores !== []) {
            $index = $this->lancamentoRomaneioFornecedorSelecionado;
            $id = (int) ($this->lancamentoRomaneioFornecedores[$index]['id']
                ?? $this->lancamentoRomaneioFornecedores[0]['id']);
            $this->selectLancamentoRomaneioFornecedor($id);

            return;
        }

        if ($value !== null) {
            $this->lancamentoRomaneioFornecedorBusca = $value;
        }
        ...
        $fornecedor = Person::query()
            ...
            ->where(function (Builder $query) use ($term): void {
                $query->where('codigo', $term)
                    ->orWhereRaw('UPPER(TRIM(nome_razao)) = ?', [mb_strtoupper($term, 'UTF-8')]);
            })
            ->orderBy('nome_razao')
            ->first();

        if ($fornecedor) {
            $this->selectLancamentoRomaneioFornecedor((int) $fornecedor->id);
        }
    }
    ...
    public function selectLancamentoRomaneioFornecedor(int $fornecedorId): void
    {
        ...
        $this->lancamentoRomaneioFornecedores = [];
        $this->lancamentoRomaneioFornecedorSelecionado = 0;
        $this->lancamentoRomaneioFornecedorConfirmado = true;
    ...
    public function addLancamentoRomaneioProduto(int $productId): void
    {
        ...
        $this->lancamentoModalRows = $this->buildLancamentoModalRows($compra, $this->lancamentoModalRows);
        $this->lancamentoModalTotais = $this->buildLancamentoModalTotais($compra);
    ...
    protected function formatLancamentoDraftSavedLabel(string $savedAt): string
    {
        ...
        try {
            return \App\Support\Erp\ErpTimezone::toLocal($savedAt)->format('d/m/Y \à\s H:i');
        } catch (\Throwable) {
            return $savedAt;
        }
    }
    ...
    /**
     * Se o rascunho não tem lotes na linha e a grade fresca (XML) tem, preserva os lotes frescos.
     *
     * @param  list<array<string, mixed>>  $draftRows
     * @param  list<array<string, mixed>>  $freshRows
     * @return list<array<string, mixed>>
     */
    protected function mergeLotesFrescosNoDraftRows(array $draftRows, array $freshRows): array
    {
        foreach ($draftRows as $i => $row) {
            ...
            $fresh = $freshRows[$i] ?? null;
            if (! is_array($fresh) || ! $this->lancamentoRowLotesTemConteudo($fresh['lotes'] ?? null)) {
                continue;
            }

            $draftRows[$i]['lotes'] = $fresh['lotes'];
            if (! array_key_exists('controla_lote_validade', $draftRows[$i]) && array_key_exists('controla_lote_validade', $fresh)) {
                $draftRows[$i]['controla_lote_validade'] = $fresh['controla_lote_validade'];
            }
        }
    ...
    protected function lancamentoRowLotesTemConteudo(mixed $lotes): bool
    {
        ...
        foreach ($lotes as $lote) {
            ...
            if (trim((string) ($lote['lote'] ?? '')) !== '' || trim((string) ($lote['data_validade'] ?? '')) !== '') {
                return true;
            }
        }
    ...
    public function finalizarCompraLancamento(): void
    {
        ...
        if ($this->lancamentoParamGeraEstoque) {
            $erroLotes = $this->validarLotesLancamentoAntesDeFinalizar();
            if ($erroLotes !== null) {
                Notification::make()
        ...
        if ($this->lancamentoRomaneio) {
            if (! $this->lancamentoRomaneioFornecedorConfirmado) {
                Notification::make()
    ...
    /**
     * Marca controla_lote_validade no produto e libera Informar sem sair do lançamento.
     */
    public function habilitarLancamentoLoteProduto(int $index): void
    {
        if ($this->lancamentoModalStatus === 'FECHADA' || $this->lancamentoModalStatus === 'CANCELADA') {
            Notification::make()
                ->title('Compra não editável')
                ->body('Abra a compra em modo Alterando para habilitar lote.')
                ->warning()
                ->send();
        ...
        if ($productId <= 0) {
            Notification::make()
                ->title('Produto não vinculado')
                ->body('Vincule um produto na linha antes de habilitar lote/validade.')
                ->warning()
                ->send();
        ...
        if (! empty($row['controla_lote_validade'])) {
            $this->abrirLancamentoLotesModal($index);
        ...
        $this->lancamentoModalRows[$index]['controla_lote_validade'] = true;

        if (! $this->lancamentoRowLotesTemConteudo($this->lancamentoModalRows[$index]['lotes'] ?? null)) {
            $xmlLotes = [];
            if ($this->lancamentoModalCompraId) {
                $compra = $this->scopedCompraQuery()->find($this->lancamentoModalCompraId);
                if ($compra) {
                    $xmlLotes = $this->lotesDoXmlPorItemCompra($compra)[$index] ?? [];
                }
            }

            if ($this->lancamentoRowLotesTemConteudo($xmlLotes)) {
                $this->lancamentoModalRows[$index]['lotes'] = $xmlLotes;
            } else {
                $this->lancamentoModalRows[$index]['lotes'] = [];
            }
        }
        ...
        Notification::make()
            ->title('Lote/validade habilitado')
            ->body('O produto passou a controlar lote. Informe os lotes desta entrada.')
            ->success()
            ->send();
    ...
    /**
     * Desfaz Habilitar: desliga controla_lote_validade no produto e volta a coluna para Habilitar.
     * Só se ainda não houver lotes preenchidos e o XML não trouxe rastro para o item.
     */
    public function desabilitarLancamentoLoteProduto(?int $index = null): void
    {
        if ($this->lancamentoModalStatus === 'FECHADA' || $this->lancamentoModalStatus === 'CANCELADA') {
            Notification::make()
                ->title('Compra não editável')
                ->body('Abra a compra em modo Alterando para desabilitar lote.')
                ->warning()
                ->send();
        ...
        $index ??= $this->lancamentoLotesRowIndex;

        if ($index === null || ! isset($this->lancamentoModalRows[$index])) {
            return;
        }

        if (! $this->lancamentoLotesPodeDesabilitarNoIndice($index)) {
            $motivo = $this->lancamentoLotesMotivoNaoDesabilitar($index);
            Notification::make()
                ->title('Não é possível desabilitar')
                ->body($motivo ?? 'Informe ou limpe os lotes antes, ou o controle veio do XML.')
                ->warning()
                ->send();
        ...
        if ($productId > 0) {
            $product = Product::query()->find($productId);
            if ($product) {
                $product->forceFill(['controla_lote_validade' => false])->save();
            }
        }

        $this->lancamentoModalRows[$index]['controla_lote_validade'] = false;
        $this->lancamentoModalRows[$index]['lotes'] = [];

        $this->fecharLancamentoLotesModal();
        $this->autosaveLancamentoDraft();

        Notification::make()
            ->title('Lote/validade desabilitado')
            ->body('O produto deixou de controlar lote nesta entrada. A coluna voltou para Habilitar.')
            ->success()
            ->send();
    }

    public function lancamentoLotesPodeDesabilitarAtual(): bool
    {
        $index = $this->lancamentoLotesRowIndex;

        return $index !== null && $this->lancamentoLotesPodeDesabilitarNoIndice($index);
    }

    protected function lancamentoLotesPodeDesabilitarNoIndice(int $index): bool
    {
        if (! isset($this->lancamentoModalRows[$index])) {
            return false;
        }
        ...
        if ($this->lancamentoLotesRowIndex === $index
            && $this->lancamentoRowLotesTemConteudo($this->lancamentoLotesDraft)) {
            return false;
        }
    ...
    protected function lancamentoLotesMotivoNaoDesabilitar(int $index): ?string
    {
        if (! isset($this->lancamentoModalRows[$index])) {
            return null;
        }
        ...
        if ($this->lancamentoModalCompraId) {
            ...
            if ($compra) {
                $xmlLotes = $this->lotesDoXmlPorItemCompra($compra)[$index] ?? [];
                if ($this->lancamentoRowLotesTemConteudo($xmlLotes)) {
                    return 'Este item veio com lote/validade no XML da nota; o controle não pode ser desligado aqui.';
                }
            }
        }

        if ($this->lancamentoRowLotesTemConteudo($row['lotes'] ?? null)
            || ($this->lancamentoLotesRowIndex === $index && $this->lancamentoRowLotesTemConteudo($this->lancamentoLotesDraft))) {
            return 'Já há lote informado nesta linha. Remova os lotes salvos antes de desabilitar.';
        }
     ...
     */
    public function abrirLancamentoLotesModal(int $index): void
    {
        if (! isset($this->lancamentoModalRows[$index])) {
            return;
        }
        ...
        $this->lancamentoLotesRowIndex = $index;
        $this->lancamentoLotesProdutoLabel = (string) ($row['produto'] ?? 'Produto');
        $this->lancamentoLotesQtdEsperada = (string) ($row['qtd'] ?? '0,000');

        $lotes = is_array($row['lotes'] ?? null) ? $row['lotes'] : [];
        if ($lotes === []) {
            $lotes = [[
                'lote' => '',
                'data_validade' => '',
                'quantidade' => (string) ($row['qtd'] ?? '0,000'),
            ]];
        }

        $this->lancamentoLotesDraft = array_values(array_map(static function ($l): array {
            return [
                'lote' => (string) ($l['lote'] ?? ''),
                'data_validade' => (string) ($l['data_validade'] ?? ''),
                'quantidade' => (string) ($l['quantidade'] ?? '0,000'),
            ];
        }, $lotes));

        $this->lancamentoLotesModalOpen = true;
    }

    public function fecharLancamentoLotesModal(): void
    {
        $this->lancamentoLotesModalOpen = false;
        $this->lancamentoLotesRowIndex = null;
        $this->lancamentoLotesDraft = [];
    }

    public function adicionarLancamentoLoteLinha(): void
    {
        $this->lancamentoLotesDraft[] = [
            'lote' => '',
            'data_validade' => '',
    ...
    public function removerLancamentoLoteLinha(int $index): void
    {
        if (! isset($this->lancamentoLotesDraft[$index])) {
            return;
        }

        unset($this->lancamentoLotesDraft[$index]);
        $this->lancamentoLotesDraft = array_values($this->lancamentoLotesDraft);

        if ($this->lancamentoLotesDraft === []) {
            $this->adicionarLancamentoLoteLinha();
        }
    }

    public function salvarLancamentoLotesModal(): void
    {
        $index = $this->lancamentoLotesRowIndex;
        if ($index === null || ! isset($this->lancamentoModalRows[$index])) {
            $this->fecharLancamentoLotesModal();
        ...
        $qtd = BrDecimal::parse($this->lancamentoModalRows[$index]['qtd'] ?? 0, 3);
        $service = new \App\Support\Erp\ProductLoteService();

        try {
            $service->validarLinhasEntrada($qtd, $this->lancamentoLotesDraft);
        ...
        $this->lancamentoModalRows[$index]['lotes'] = array_values(array_map(static function (array $l): array {
            return [
                'lote' => trim((string) ($l['lote'] ?? '')),
                'data_validade' => trim((string) ($l['data_validade'] ?? '')),
        ...
        Notification::make()
            ->title('Lotes gravados na linha')
            ->success()
            ->send();
    }

    protected function validarLotesLancamentoAntesDeFinalizar(): ?string
    {
        $service = new \App\Support\Erp\ProductLoteService();
        ...
        foreach ($this->lancamentoModalRows as $row) {
            ...
            $qtd = BrDecimal::parse($row['qtd'] ?? 0, 3);
            if ($qtd <= 0) {
                continue;
            }

            $lotes = is_array($row['lotes'] ?? null) ? $row['lotes'] : [];
            $produto = (string) ($row['produto'] ?? 'produto');

            try {
                $service->validarLinhasEntrada($qtd, $lotes);
            } catch (\RuntimeException $e) {
                return $produto.': '.$e->getMessage();
            }
        }
    ...
    /**
     * @param  list<array<string, mixed>>|null  $preserveFrom  linhas atuais (não sobrescrever lotes já informados)
     * @return list<array<string, mixed>>
     */
    protected function buildLancamentoModalRows(Compra $compra, ?array $preserveFrom = null): array
    {
        $rows = [];
        $index = 1;
        ...
        foreach ($compra->itens as $item) {
            ...
            if ($controlaLote) {
                $preserveKey = $productId.'#'.$rowIndex;
                if (isset($preservedByKey[$preserveKey])) {
                    $lotes = $preservedByKey[$preserveKey];
                } elseif ($temLotesXml) {
                    $lotes = $xmlLotes;
                }
            }
    ...
    /**
     * @return array<int, list<array{lote: string, data_validade: string, quantidade: string}>>
     */
    protected function lotesDoXmlPorItemCompra(Compra $compra): array
    {
        $nota = NotaFornecedor::query()
        ...
        foreach ($itens as $i => $item) {
            ...
            $lotes = is_array($item['lotes'] ?? null) ? $item['lotes'] : [];
            $normalizados = [];
            ...
            foreach ($lotes as $lote) {
                ...
                $codigoLote = trim((string) ($lote['lote'] ?? ''));
                $validade = trim((string) ($lote['data_validade'] ?? ''));
                $quantidade = trim((string) ($lote['quantidade'] ?? ''));

                if ($codigoLote === '' && $validade === '' && ($quantidade === '' || $quantidade === '0,000')) {
                    continue;
                }
                ...
                $normalizados[] = [
                    ...
                    'quantidade' => $quantidade !== '' ? $quantidade : '0,000',
                ];
            }

            $byIndex[(int) $i] = $normalizados;
        }
     ...
     * @return array<string, list<array{lote: string, data_validade: string, quantidade: string}>>
     */
    protected function indexPreservedLancamentoLotes(?array $rows): array
    {
        if ($rows === null || $rows === []) {
            return [];
        }
        ...
        foreach ($rows as $i => $row) {
            ...
            foreach ($lotes as $lote) {
                ...
                $codigoLote = trim((string) ($lote['lote'] ?? ''));
                $validade = trim((string) ($lote['data_validade'] ?? ''));
                $quantidade = trim((string) ($lote['quantidade'] ?? '0,000'));
                $normalizados[] = [
                    'lote' => $codigoLote,
                    'data_validade' => $validade,
                    'quantidade' => $quantidade !== '' ? $quantidade : '0,000',
            ...
            $productId = (int) ($row['product_id'] ?? 0);
            $indexed[$productId.'#'.(int) $i] = $normalizados;
        }
    ...
    /**
     * Rateia frete + seguro + outras + ST + IPI no V. Custo (preço de compra) de cada item,
     * e desconta o desconto proporcional. Usa vl_custo_base para não acumular em edições.
     */
    protected function aplicarRateioExtrasNoVlCusto(): void
    {
        ...
        $acrescimos = BrDecimal::parse($this->lancamentoModalTotais['frete'] ?? 0, 2)
            + BrDecimal::parse($this->lancamentoModalTotais['seguro'] ?? 0, 2)
            + BrDecimal::parse($this->lancamentoModalTotais['outras'] ?? 0, 2)
            + BrDecimal::parse($this->lancamentoModalTotais['valor_st'] ?? 0, 2)
            + BrDecimal::parse($this->lancamentoModalTotais['valor_ipi'] ?? 0, 2);
        ...
        foreach ($this->lancamentoModalRows as $index => &$row) {
            ...
            if ($baseSum <= 0 || $qtd <= 0) {
                $row['vl_custo'] = number_format(max(0.0, $baseCusto), 2, ',', '.');

                continue;
            }

            $proporcao = $totalLinha / $baseSum;

            if ($acrescimos > 0) {
                if ($index === $lastIndex) {
                    $shareAcrescimo = round($acrescimos - $rateadoAcrescimo, 2);
                } else {
                    $shareAcrescimo = round($acrescimos * $proporcao, 2);
                    $rateadoAcrescimo += $shareAcrescimo;
                }
            } else {
                $shareAcrescimo = 0.0;
            }
            ...
            if ($desconto > 0) {
                if ($index === $lastIndex) {
                    $shareDesconto = round($desconto - $rateadoDesconto, 2);
                } else {
                    $shareDesconto = round($desconto * $proporcao, 2);
                    $rateadoDesconto += $shareDesconto;
                }
            } else {
                $shareDesconto = 0.0;
            }

            $ajusteUnitario = round(($shareAcrescimo - $shareDesconto) / $qtd, 4);
            $row['vl_custo'] = number_format(max(0.0, $baseCusto + $ajusteUnitario), 2, ',', '.');
        }
        unset($row);
    ...
    public function cancelCompra(): void
    {
        ...
        if ($compra->status === Compra::STATUS_FECHADA) {
            Notification::make()
                ->title('Compra fechada')
                ->body('Use F7 Reabrir para estornar estoque/financeiro; depois cancele.')
                ->warning()
                ->send();
        ...
        Notification::make()
            ->title('Compra cancelada.')
            ->body('Se havia nota vinculada, ela voltou para Aceita e pode gerar compra novamente.')
            ->success()
            ->send();
    }
